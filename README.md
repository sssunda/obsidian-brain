...
Claude Code로 일하다 보면 좋은 판단, 시행착오, 결정의 이유가 대화 속에 묻힌다. Obsidian Brain은 그 대화를 자동으로 분석해서 **"나는 왜 이런 선택을 하는 사람인가"**를 알려주는 도구.

설치하면 끝. 세션 끝날 때마다 알아서 기록하고, 매일 종합해서 나의 의사결정 패턴을 정리해준다. Pro/Max 구독 안에서 동작하므로 추가 API 비용 없음.
